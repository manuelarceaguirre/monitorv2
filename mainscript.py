        ...
        if isinstance(drift_results, dict) and 'tests' in drift_results:
            for test in drift_results['tests']:
                if isinstance(test, dict):
                    ...
                    # --- Fix: Extract p-value and threshold correctly ---
                    p_value = result.get('p_value')  # Use .get() to avoid KeyError
                    ...
                    drift_data.append({
                        ...
                        'p_value': p_value,  # Store the extracted p-value
                        'threshold': threshold,
                        'drifted': drifted,
                        'Timestamp': timestamp,
                        'Batch': month
                    })
                    logger.info(f"Added drift data for feature: {column}, p-value: {p_value}, drifted: {drifted}")

        # ... (Rest of the save_results function remains the same)
...
def create_summary(drift_results: Dict) -> str:
    ...
    try:
        ...
        # --- Improved Summary: Include All Features ---
        ...
        for test in test_suite_results:
            ...
            column = test['parameters']['column_name']
            result = test['result']
            drifted = result.get('drift_detected', 'N/A')  # Handle missing keys
            p_value = result.get('p_value', 'N/A')  # Handle missing keys
            threshold = result.get('threshold', 'N/A')  # Handle missing keys
            test_type = test['parameters']['stattest']
        ...
        overall_drifted = any(test['result'].get('drift_detected', False) for test in test_suite_results if isinstance(test, dict))
        drifted_columns_count = sum(1 for test in test_suite_results if isinstance(test, dict) and test['result'].get('drift_detected', False))
        total_columns_count = len(test_suite_results)
...
def analyze_drift(reference_data: pd.DataFrame, current_data: pd.DataFrame, config: Dict) -> Dict:
    ...
    try:
        ...
        for column, cfg in columns_config.items():
            ...
            logger.info(f"Processing column {column} (type: {col_type}) using {test_type} test")
            ...
            try:
                ...
                if col_type == 'categorical':
                    ...
                    logger.info(f"Categorical column {column} processed with {len(all_categories)} categories")
                else:
                    # Ensure numerical columns are float
                    reference_data[column] = reference_data[column].astype('float64')
                    current_data[column] = current_data[column].astype('float64')
                
                # Create test for this column
                test = TestColumnDrift(
                    column_name=column,
                    stattest=test_type
                )
                tests.append(test)
                ...
                logger.info(f"Column {column} stats - "
                            f"ref unique: {len(reference_data[column].unique())}, "
                            f"cur unique: {len(current_data[column].unique())}, "
                            f"ref dtype: {reference_data[column].dtype}, "
                            f"cur dtype: {current_data[column].dtype}")
        ...
        if not tests:
            logger.error("No valid tests configured")
            return {'tests': []}
            
        logger.info(f"Created {len(tests)} drift tests")
        ...
        # Print out full test results for debugging
        # logger.info(json.dumps(test_results, indent=2))
        
        if 'tests' in test_results:
            for test in test_results['tests']:
                if 'parameters' in test and 'column_name' in test['parameters']:
                    column = test['parameters']['column_name']
                    test_type = per_column_stattest.get(column)
                    threshold = config['drift_tests']['drift_thresholds']['test_thresholds'].get(test_type, 0.05)
                    
                    # Extract metrics instead of result
                    metrics = test.get('metrics', {})
                    drift_detected = metrics.get('drifted', False)
                    ...
                    logger.info(f"Processed drift results for {column}: "
                                f"drift_detected={drift_detected}, "
                                f"p_value={p_value}, "
                                f"test_type={test_type}, threshold={threshold}")
        
        logger.info(f"Drift analysis completed. Processed {len(drift_results['tests'])} columns")
